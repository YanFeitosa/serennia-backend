...
app.delete("/category/:id", async (req, res) => {
    try {
        ...
        try {
            ...
            tx.push(prisma.service.updateMany({
                where: { salonId, categoryId: existing.id },
                data: { categoryId: null },
            }));
            tx.push(prisma.product.updateMany({
                where: { salonId, categoryId: existing.id },
                data: { categoryId: null },
            }));
            ...
            if (existing.type === "service") {
                ...
                for (const collaborator of collaborators) {
                    ...
                    tx.push(prisma.collaborator.update({
                        where: { id: collaborator.id },
                        data: { serviceCategories: filteredCategories },
                    }));
                }
            }
            tx.push(prisma.category.delete({
                where: { id: existing.id },
            }));
...
app.delete("/orders/:id/items/:itemId", async (req, res) => {
    try {
        const salonId = await getDefaultSalonId();
        const result = await prisma.$transaction(async (tx) => {
            const order = await tx.order.findFirst({
...
app.post("/appointments/:id/order/ensure", async (req, res) => {
    try {
        const salonId = await getDefaultSalonId();
        const appointment = await prisma.appointment.findFirst({
            where: { id: req.params.id, salonId },
