...
registerRouter.post("/", registerRateLimiter, async (req, res) => {
    try {
        console.log("Registration request received:", {
            body: { ...req.body, password: req.body.password ? '[REDACTED]' : undefined },
            headers: req.headers,
        });
        const { salonName, name, email, phone, password } = req.body;
        ...
        const sanitizedSalonName = (0, validation_1.sanitizeString)(salonName);
        ...
        const sanitizedPhone = (0, validation_1.sanitizeString)(phone);
        ...
        let existingDbUser;
        try {
            existingDbUser = await prismaClient_1.prisma.user.findFirst({
        ...
        try {
            ...
            if (listError) {
                ...
                const emailExists = existingUsers.users.some(u => u.email?.toLowerCase() === sanitizedEmail.toLowerCase());
                if (emailExists) {
        ...
        const createResult = await supabase_1.supabaseAdmin.auth.admin.createUser({
        ...
        if (authError && authError.message.includes('Database error')) {
            ...
            const retryResult = await supabase_1.supabaseAdmin.auth.admin.createUser({
                email: sanitizedEmail,
                password: password,
                email_confirm: true,
                user_metadata: {
        ...
        if (authError && authError.message.includes('Database error')) {
            ...
            const finalResult = await supabase_1.supabaseAdmin.auth.admin.createUser({
                email: sanitizedEmail,
                password: password,
                email_confirm: true,
                user_metadata: {
        ...
        if (authError && !authData?.user) {
            ...
            if (errorMessage.includes('Database error') || errorCode === 'unexpected_failure') {
                ...
                try {
                    ...
                    const { data: checkUser, error: checkError } = await supabase_1.supabaseAdmin.auth.admin.listUsers({
                        page: 1,
                        perPage: 1000,
                    });
                    if (!checkError && checkUser?.users) {
                        const userExists = checkUser.users.find(u => u.email?.toLowerCase() === sanitizedEmail.toLowerCase());
                        if (userExists) {
        ...
        let salon;
        try {
            salon = await prismaClient_1.prisma.salon.create({
                data: {
                    name: sanitizedSalonName,
        ...
        try {
            ...
            if (userError.code === 'P2022' || userError.message?.includes('does not exist')) {
                ...
                try {
                    ...
                    try {
                        await prismaClient_1.prisma.salon.delete({ where: { id: salon.id } });
        ...
        try {
            const { error: updateError } = await supabase_1.supabaseAdmin.auth.admin.updateUserById(authData.user.id, {
                user_metadata: {
                    salonId: salon.id,
                    platformRole: "tenant_admin",
        ...
        // Send welcome email
        try {
            await (0, email_1.sendWelcomeEmail)(sanitizedEmail, sanitizedName, sanitizedSalonName);
        }
        catch (emailError) {
