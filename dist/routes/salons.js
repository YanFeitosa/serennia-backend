...
exports.salonsRouter = void 0;
const express_1 = require("express");
const prismaClient_1 = require("../prismaClient");
const salonsRouter = (0, express_1.Router)();
exports.salonsRouter = salonsRouter;
...
salonsRouter.get('/', async (req, res) => {
    try {
        ...
        const salons = await prismaClient_1.prisma.salon.findMany({
            select: {
                id: true,
                name: true,
                document: true,
...
salonsRouter.post('/:id/select', async (req, res) => {
    try {
        if (!req.user) {
            res.status(401).json({ error: 'Unauthorized' });
            return;
