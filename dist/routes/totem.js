...
totemRouter.post('/client/login', async (req, res) => {
    try {
...
totemRouter.post('/client/register', async (req, res) => {
    try {
        const salonId = await (0, salonContext_1.getDefaultSalonId)();
        ...
        if (existing) {
            if (existing.isActive) {
                ...
                const reactivated = await prismaClient_1.prisma.client.update({
...
totemRouter.get('/services', async (req, res) => {
    try {
        const salonId = await (0, salonContext_1.getDefaultSalonId)();
...
totemRouter.get('/collaborators', async (req, res) => {
    try {
        const salonId = await (0, salonContext_1.getDefaultSalonId)();
        const { serviceCategoryIds } = req.query;
        const where = {
            salonId,
            status: 'active',
        };
        ...
        if (serviceCategoryIds) {
            const categoryIds = serviceCategoryIds.split(',');
...
totemRouter.get('/availability', async (req, res) => {
    try {
        const salonId = await (0, salonContext_1.getDefaultSalonId)();
        const { collaboratorId, date, duration } = req.query;
...
totemRouter.post('/appointments', async (req, res) => {
    try {
        const salonId = await (0, salonContext_1.getDefaultSalonId)();
        const { clientId, collaboratorId, serviceIds, start } = req.body;
        ...
        // Verificar conflitos
        const overlapping = await prismaClient_1.prisma.appointment.findFirst({
            where: {
                salonId,
                collaboratorId,
                status: { in: APPOINTMENT_BLOCKING_STATUSES },
